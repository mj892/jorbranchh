...
if (isset($_GET['name'], $_GET['quantity'], $_GET['cellphone'], $_GET['address'], $_GET['payment'], $_GET['fruitName'])) {
    ...
    // Set the fruit price based on the fruit name
    if ($fruitName === 'Ficus bonsai') {
        $fruitPrice = 850;
        $driverName = "Marco Polo";
        $driverPlateNumber = "XYZ 1234";
    } elseif ($fruitName === 'Carmona bonsai') {
        $fruitPrice = 1500;
        $driverName = "Joaldin Lupaypay";
        $driverPlateNumber = "TMB 136";
    } elseif ($fruitName === 'Juniper bonsai') {
        $fruitPrice = 1200;
        $driverName = "Steph Matata";
        $driverPlateNumber = "NYC 145";
    } else {
        $fruitPrice = 0; // Default price if the fruit name is not recognized
    }
    ...
    echo "<p><strong>Bonsai Name:</strong> $fruitName</p>";
    echo "<p><strong>Bonsai Price:</strong> Php $fruitPrice</p>"; 
